...
function createBigIntTypedArrayNode(id, current, buffer) {
	return createSerovalNode(16, id, NIL, current.constructor.name, NIL, NIL, NIL, NIL, buffer, current.byteOffset, NIL, current.length);
}
...
function serializeProperty(ctx, source, key, val) {
	if (typeof key === "string") {
		...
		if (isValidKey(key)) return (isIdentifier ? key : "\"" + key + "\"") + ":" + serialize(ctx, val);
